...
server <- function(input, output, session){
  ...
  observeEvent(input$apply_filters, {
    ...
    if (input$quarter_filter != "all") {
      df <- filter(df, QuarterLabel == input$quarter_filter)
    }
    if (input$outcome_filter != "all"){
      df <- filter(df, ShotOutcome == input$outcome_filter)
    }
    
    if (!is.null(input$range1)) {
      v1 <- input$number1
      df <- df[df[[v1]] >= input$range1[1] & df[[v1]] <= input$range1[2], ]
    }
    
    if (!is.null(input$range2)) {
      v2 <- input$number2
      df <- df[df[[v2]] >= input$range2[1] & df[[v2]] <= input$range2[2], ]
    }
