...
### Assigning Point Values for 3-pointers, 2-pointers, and free throws. Re-coding Quarters for better representation.
```{r}
...
#Re-coding Quarters (Q1-Q4, OT1-OT4)
pbp_condensed <- pbp_condensed |>
  mutate(QuarterLabel = case_when(
    Quarter == 1 ~ "Q1",
    Quarter == 2 ~ "Q2",
    Quarter == 3 ~ "Q3",
    Quarter == 4 ~ "Q4",
    ...
    TRUE ~ as.character(Quarter))) |>
      mutate(QuarterLabel = factor(QuarterLabel, levels = c("Q1", "Q2", "Q3", "Q4", "OT1", "OT2", "OT3", "OT4")))
...
#Quarter Distribution (5-8 = Overtime)
table(pbp_condensed$Quarter)
```

### Two-Way Contigency Tables
```{r}
#Shot Outcome x Quarter
addmargins(table(pbp_condensed$ShotOutcome, pbp_condensed$Quarter, useNA = "no"))

#Home Team x Quarter (The amount of offensive plays run by the Home team per Quarter)
addmargins(table(pbp_condensed$HomeTeam, pbp_condensed$Quarter))

#Away Team x Quarter (The amount of offensive plays run by the Away team per Quarter)
addmargins(table(pbp_condensed$AwayTeam, pbp_condensed$Quarter))
```
...
### Shot Outcome by Quarter and Season
```{r}
...
ggplot(filter(pbp_condensed, !is.na(ShotOutcome)), 
  ...
  facet_wrap(~ year) + 
  labs(title = "Shot Oucomes by Quarter and Season", x = "Quarter", y = "Proportion")
```
